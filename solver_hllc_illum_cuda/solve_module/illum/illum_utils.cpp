...
int SolveIllumAndHLLC(const Type tau, std::vector<elem_t>& cells)
{
	...
	{
		//el.phys_val.d += 0;
		el.phys_val.v += tau * (-el.illum_val.div_impuls * C_LIGHT_INV);  //- tau*(stream[i][0] - prev_stream[i][0]) / tau / c / c);  // +F_g //vx
		el.phys_val.p += tau * (-el.illum_val.div_stream); //  tau*(-(energy[i] - prev_energy[i]) / tau / c)  //+F_g.dot(vel)  //e
	}
