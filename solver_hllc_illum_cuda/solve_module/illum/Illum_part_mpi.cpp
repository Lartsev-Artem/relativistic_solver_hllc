...
{	
	...
	if (myid == 0)
	{
		...
		{			
			...
			// сюда же absorp_coef, если понадобитьс€
		}
	...
	do {
		...
		{
			//inter_coef.assign(inter_coef.size(), Vector3::Zero()); //была ситуаци€ не инициализации границы. Ћучше уж ноль, чем мусор
			...
			for (int h = 0; h < count_cells; ++h)
			{
				...
				{					
					...
					{
						//формально обрабатываем мы только выход€щие грани, т.к. вход€щие к этому моменту определены,
						// но границу надо инициализировать. Ќе дл€ расчета, но дл€ конечного интегрировани€
						if (out_id_face < 0) 
						{		
